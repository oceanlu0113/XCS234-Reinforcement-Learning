...
class QN(object):
    ...
    def train(self, exp_schedule, lr_schedule):
        ...
        replay_buffer = ReplayBuffer(self.config["hyper_params"]["buffer_size"], self.config["hyper_params"]["state_history"])
        ...
        while t < self.config["hyper_params"]["nsteps_train"]:
            ...
            while True:
                ...
                last_eval += 1
                last_record += 1
                if self.config["env"]["render_train"]: self.env.render()
                # replay memory stuff
                ...
                # logging stuff
                if ((t > self.config["hyper_params"]["learning_start"]) and (t % self.config["model_training"]["log_freq"] == 0) and
                   (t % self.config["hyper_params"]["learning_freq"] == 0)):
                    ...
                    if len(rewards) > 0:
                        prog.update(t + 1, exact=[("Loss", loss_eval), ("Avg_R", self.avg_reward), 
                                        ...
                                        ("lr", lr_schedule.epsilon)], base=self.config["hyper_params"]["learning_freq"])
                    self.timer.end('logging')
                elif (t < self.config["hyper_params"]["learning_start"]) and (t % self.config["model_training"]["log_freq"] == 0):
            ...
            if (t > self.config["hyper_params"]["learning_start"]) and (last_eval > self.config["model_training"]["eval_freq"]):
                # evaluate our policy
                last_eval = 0
                print("")
            ...
            if (t > self.config["hyper_params"]["learning_start"]) and self.config["env"]["record"] and (last_record > self.config["model_training"]["record_freq"]):
    ...
    def train_step(self, t, replay_buffer, lr):
        ...
        # perform training step
        if (t > self.config["hyper_params"]["learning_start"] and t % self.config["hyper_params"]["learning_freq"] == 0):
    ...
    def evaluate(self, env=None, num_episodes=None):
        ...
        for i in range(num_episodes):
            ...
            state = env.reset()
            while True:
                if self.config["env"]["render_test"]: env.render()
    ...
    def record(self):
        ...
        env = gym.wrappers.Monitor(env, self.config["output"]["record_path"], video_callable=lambda x: True, resume=True)
