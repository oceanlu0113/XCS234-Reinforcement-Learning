...
class Linear(DQN):
    ...
    def initialize_models(self):
        ...
        channels = n_channels * config["hyper_params"]["state_history"]
        image = img_height * img_width * channels
        
        # nn.Linear(H_in, H_out)
        self.q_network = nn.Linear(image, num_actions)
        self.target_network = nn.Linear(image, num_actions)
    ...
    def get_q_values(self, state, network='q_network'):
        ...
        ## input dim: batch_size x (img_height * img_width * n_channels * state_history)

        state_flattened = torch.flatten(state,start_dim=1)
        
        if network == 'q_network':
            return self.q_network(state_flattened)
        else:
            return self.target_network(state_flattened)
        
        ## output dim: batch_size x num_actions
    ...
    def calc_loss(self, q_values : torch.Tensor, target_q_values : torch.Tensor,
        ...
        best_target_q = torch.max(target_q_values)
        Q_samp = rewards + (1. - done_mask.type(torch.float)) * gamma * best_target_q
        Q_actions = F.one_hot(actions.long(), num_classes = num_actions)
        Q_sa = torch.sum(q_values * Q_actions, axis=1)
