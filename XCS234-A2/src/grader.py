...
class Test_3a(GradedTestCase):
    ...
    def test_1(self):
        ...
        exp_strat = submission.LinearExploration(env, 1, 0, 10)
        exp_strat.update(5)
        self.assertEqual(exp_strat.epsilon, 0.5)
...
class Test_5b(GradedTestCase):
    ...
    def test_0(self):
        ...
        self.assertEqual(model.q_network.weight.size(),
            torch.Size([num_actions,
                img_height * img_width * n_channels * q5_config["hyper_params"]["state_history"]]))
        self.assertTrue(isinstance(model.target_network, nn.Linear))
        self.assertEqual(model.target_network.weight.size(),
            torch.Size([num_actions,
                img_height * img_width * n_channels * q5_config["hyper_params"]["state_history"]]))
...
class Test_5e(GradedTestCase):
    ...
    def test_0(self):
        ...
        state = torch.full((1, img_height, img_width, n_channels * q5_config["hyper_params"]["state_history"]), 0.5)
        ...
        self.assertEquals(round(loss.item(), 3), 424.442)
...
class Test_5f(GradedTestCase):
    ...
    def test_0(self):
        ...
        self.assertTrue(model.optimizer.param_groups[0]['params'][0] is model.q_network.weight)
        self.assertTrue(model.optimizer.param_groups[0]['params'][1] is model.q_network.bias)
...
class Test_6a(GradedTestCase):
    ...
    def test_0(self):
        ...
        img_height, img_width, n_channels = state_shape
        num_actions = env.action_space.n
        sample_input = torch.randn(1, img_height, img_width, n_channels * q6_config["hyper_params"]["state_history"])
