...
def main(args):
	...
	if args.model == "linucb": 
		frac_incorrect = run(args, data, submission.LinUCB(3, features, alpha=args.alpha), large_error_penalty=args.large_error_penalty)

	if args.model == "egreedy": 
		frac_incorrect = run(args, data, submission.eGreedyLinB(3, features, alpha=args.ep), large_error_penalty=args.large_error_penalty)

	if args.model == "thompson": 
		frac_incorrect = run(args, data, submission.ThomSampB(3, features, alpha=args.v2), large_error_penalty=args.large_error_penalty)
	...
	for filename in os.listdir('results'):
		if filename.endswith('.csv'):
			...
			with open(os.path.join('results', filename), 'r') as f:
				frac_incorrect_all.append((algorithm, np.array(list(csv.reader(f))).astype('float64').squeeze()))			
