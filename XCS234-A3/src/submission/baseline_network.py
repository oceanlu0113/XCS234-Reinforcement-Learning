...
class BaselineNetwork(nn.Module):
    ...
    def __init__(self, env, config):
        ...
        # create self.network
        self.network = build_mlp(input_size = self.env.observation_space.shape[0], output_size = 1,
    ...
    def calculate_advantage(self, returns, observations):
        ...
        if baseline.device != "cpu":
            baseline = baseline.to("cpu")
        # convert torch to numpy
        baseline = baseline.detach().numpy() 
    ...
    def update_baseline(self, returns, observations):
        ...
        predict = self(observations)
        los = self.loss(predict, returns)
        # back prop
        los.backward()
        # calculate step
        self.optimizer.step()
