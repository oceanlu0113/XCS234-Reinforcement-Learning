...
class PolicyGradient(object):
    ...
    def __init__(self, env, config, seed, logger=None):
        ...
        if config["model_training"]["use_baseline"]:
            self.baseline_network = BaselineNetwork(env, config).to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    def init_policy(self):
        ...
        self.network = build_mlp(input_size = self.observation_dim, output_size = self.action_dim,
                                     n_layers=self.config["hyper_params"]["n_layers"],size = self.config["hyper_params"]["layer_size"])
        
        if self.discrete:
            # instantiate a CategoricalPolicy(BasePolicy, nn.Module)
            self.policy = CategoricalPolicy(self.network)
        else: 
            # instantiate a GaussianPolicy(BasePolicy, nn.Module)
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            #    returns[i] += self.config["hyper_params"]["gamma"] * returns[i + 1]
            ...
            for time_step in range(episode_len):
                ...
                gammas = np.logspace(0, tail_length, num=tail_length,
                                     base=self.config["hyper_params"]["gamma"], endpoint=False)
    ...
    def train(self):
        """
        Performs training, you do not have to change or use anything here, butit is worth taking 
        ...
        self.policy = self.policy.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        ...
        if self.evaluate(self.env, num_episodes=self.config["model_training"]["num_episodes_eval"]) >= self.config["env"]["min_expected_reward"]:
            torch.save(self.policy.state_dict(), "submission/{}-{}-model-weights.pt".format(self.config["env"]["env_name"], "baseline" if self.config["model_training"]["use_baseline"] else "no-baseline"))
            np.save("submission/{}-{}-scores.npy".format(self.config["env"]["env_name"], "baseline" if self.config["model_training"]["use_baseline"] else "no-baseline"), averaged_total_rewards)
        
        torch.save(self.policy.state_dict(), self.config["output"]["model_output"].format(self.seed))
        np.save(self.config["output"]["scores_output"].format(self.seed), averaged_total_rewards)
        export_plot(averaged_total_rewards, "Score", self.config["env"]["env_name"], self.config["output"]["plot_output"].format(self.seed))
    ...
    def record(self):
        ...
        env.seed(self.seed)
        env = gym.wrappers.Monitor(env, self.config["output"]["record_path"].format(self.seed), video_callable=lambda x: True, resume=True)
        self.evaluate(env, 1)
