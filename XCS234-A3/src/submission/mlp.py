...
def build_mlp(
    ...
    """
    ### START CODE HERE ###
    # multilayer perceptron based on input argument values
    layers = []
    mySize = input_size
    # n_layers, each of 'size' units
    for _ in range(n_layers):
        # maps input_size-dimensional vectors to output_size-dimensional vectors
        layers.append(nn.Linear(mySize, size))
        # followed by ReLu nonlinearity
        layers.append(nn.ReLU())
        mySize = size
    layers.append(nn.Linear(size, output_size))
    # Build a feed-forward network, linear, no ReLu
