...
if __name__ == '__main__':
    ...
    if args.plot_config_filename is not None:
        ...
        for env in config.keys():
            ...
            for seed in config[env]["seed"]:
                baseline_directory = "./results/{}-{}-baseline/".format(gym_env_name, seed)
                no_baseline_directory = "./results/{}-{}-no-baseline/".format(gym_env_name, seed)
                if not os.path.isdir(no_baseline_directory):
                    sys.exit("{} was not found. Please ensure you have generated results for this environment, seed and baseline combination".format(no_baseline_directory))
                if not os.path.isdir(no_baseline_directory):
                    sys.exit("{} was not found. Please ensure you have generated results for this environment, seed and baseline combination".format(baseline_directory))
                all_results['Baseline'].append(np.load(baseline_directory + 'scores.npy'))
                all_results['No baseline'].append(np.load(no_baseline_directory + 'scores.npy'))
