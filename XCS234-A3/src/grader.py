...
class Test_1g(GradedTestCase):
    ...
    def test_0(self):
        ...
        with torch.no_grad():
            log_probs = policy.action_distribution(observations).log_prob(actions).cpu().numpy()
            ref_log_probs = ref_policy.action_distribution(observations).log_prob(actions).cpu().numpy()
